...
/**
 * Class ContactEventsDatatable
 * @package Modules\Contacts\Http\Controllers\Tabs
 */
class ContactEventsDatatable extends RelationDataTable
{
    const SHOW_URL_ROUTE = 'calendar.events.show';

    protected $unlinkRoute = 'contacts.events.unlink';

    protected $deleteRoute = 'contacts.events.delete';
    ...
    public function query(Event $model)
    {
        $query = $model->newQuery()
            ...
            ->leftJoin('contacts', 'bap_calendar_event.contact_id', '=', 'contacts.id')
            ->select(
                'bap_calendar_event.*',
