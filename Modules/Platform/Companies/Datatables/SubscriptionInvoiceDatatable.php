...
class SubscriptionInvoiceDatatable extends PlatformDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable->filterColumn('created_at', function ($query, $keyword) {
            ...
            if ($dates != null) {
                $query->whereBetween('bap_subscription_invoice.created_at', array($dates[0], $dates[1]));
            }
        });
        ...
        $dataTable->filterColumn('updated_at', function ($query, $keyword) {
            ...
            if ($dates != null) {
                $query->whereBetween('bap_subscription_invoice.updated_at', array($dates[0], $dates[1]));
            }
        });
        ...
        $dataTable->filterColumn('status', function ($query, $keyword) {
            ...
            if ($keyword == 'yes') {
                $query->where('bap_subscription_invoice.status', 1);
            } else {
                $query->where('bap_subscription_invoice.status', 0);
            }
        });
    ...
    public function query(SubscriptionInvoice $model)
    {
        ...
        leftJoin('bap_companies', 'bap_subscription_invoice.company_id', '=', 'bap_companies.id')
            ->select([
                'bap_subscription_invoice.*',
                'bap_companies.name as company_name'
            ]);
    }
